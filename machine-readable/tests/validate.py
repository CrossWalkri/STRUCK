#!/usr/bin/env python3
"""Enforce STRUCK's structural obligations: the conformant example must validate against
dist/struck.schema.json and every non-conformant fixture must fail. Run against the
GENERATED JSON Schema with a standard 2020-12 validator (not LinkML's own), which is where
the conditional obligations are actually enforced. Exit 0 if all pass, 1 otherwise."""
import json, sys
from pathlib import Path
import yaml, jsonschema

HERE = Path(__file__).resolve().parent.parent
schema = json.loads((HERE / "dist" / "struck.schema.json").read_text())
ok = True
def must_pass(p):
    global ok
