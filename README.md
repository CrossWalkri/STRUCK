...
The exit-boundary standard: what an output owes before anyone acts on it. STRUCK is the last legible commitment a chain makes about a finding before it travels.

Every system that turns records into decisions eventually hands a finding to someone who acts on it. Everything before that point is process; everything after it is consequence. STRUCK specifies what a finding must carry on its face so that the party receiving it can do something other than take it or leave it: check it, and if it is wrong, show it wrong. It specifies obligations, not implementations, and binds no one to any tool, format, or vocabulary. A finding produced entirely by hand can satisfy it as fully as one produced by a pipeline.
...
A finding conforms when it carries all five obligations, or explicitly declares and justifies the absence of any that does not apply. Conformance is assessable from the finding alone, with no access to the process that produced it.

- **Support graded.** The finding exposes the grade profile of its material support per dimension, and never collapses it into a single combined confidence figure.
- **Refutation stated.** The finding states, in terms of what could be found in the world, what would overturn it.
- **Contest kept.** Where the support disagrees, the finding represents the disagreement rather than resolving it into an average that no source asserts.
...
- **Unjudged worth.** The finding leaves the judgment of sufficiency to the party consuming it, and carries what that party needs to make it.

The line STRUCK holds is between certifying the material and judging the use: it certifies that a finding met its prestated intended use, and says nothing about whether that use was the right one. Certify the material, never judge the art.
...
STRUCK and ORE are one handoff seen from two sides. STRUCK is what an upstream chain owes on the way out; ORE is what a downstream chain may assume on the way in. A STRUCK-conformant finding is exactly the thing ORE grades well, because it arrives already carrying its grade profile, its refutation conditions, and its derivation walked to origin.
